...
def on_connect(client, unused_userdata, unused_flags, rc):
    ...
    client.subscribe('/devices/{}/commands/#'.format(device_id) ,qos=1)
...
def main():
    ...
    while True:
        ...
        if action == 'event':
            ...
            payload = '{{"Month": "{}", "Day": "{}", "Time": "{}", "Temp1": {}, "Hum1": {}, "Room1": {}, "Temp2": {}, "Hum2": {}, "Room2": {} }}'.format(command["Month"],command["Day"],command["Time"],command["Temp1"],command["Hum1"],command["Room1"],command["Temp2"],command["Hum2"],command["Room2"])
        ...
        if action == 'motor':
            print('Sending telemetry event for device {}'.format(device_id))
            payload = '{{"data": {} }}'.format(command["data"])
            mqtt_topic = '/devices/{}/commands/'.format(device_id)
            print('Publishing message to topic {} with payload \'{}\''.format(
                    mqtt_topic, payload))
            _, event_mid = client.publish(mqtt_topic, payload,qos=0)

            message = template.format(device_id, 'command')
            udpSerSock.sendto(message.encode('utf8'), client_addr)
            ...
            print('subscribe config for {}'.format(device_id))
            subscribes_topic = '/devices/{}/commands/#'.format(device_id)
            skip_next_sub = True
            
            _, mid = client.subscribe(subscribes_topic ,qos=1)
            message = template.format(device_id, 'subscribe')
            gateway_state.subscriptions[subscribes_topic] = client_addr
